...
module.exports = function(sequelize, DataTypes) {
  ...
  var User = sequelize.define("User", {
      ...
      admin : {
          ...
          get: function(){
            return Boolean(this.getDataValue('admin'));
          },
      },
      auto_approve : {
        type         : DataTypes.BOOLEAN,
        ...
        get: function(){
          return Boolean(this.getDataValue('auto_approve'));
        },
      },
      start_date : {
          type         : DataTypes.DATE,
...
function get_instance_methods(sequelize) {
  ...
  return {
    ...
    get_reset_password_token : function(){
      ...
      return Buffer.from( self.email + ' ' + self.constructor.hashify_password( self.password ) ).toString('base64');
    },
...
function get_class_methods(sequelize) {
  return {
    ...
    get_user_by_reset_password_token : function(token) {
      var self                  = this,
      unpacked_token            = Buffer.from(token, 'base64').toString('ascii'),
