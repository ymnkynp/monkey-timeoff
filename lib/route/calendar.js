...
router.get('/', function(req, res) {
  ...
  var show_full_year = req.query['show_full_year'] ? validator.toBoolean(req.query['show_full_year']) : false;
...
router.get('/teamview/', async (req, res) => {
  ...
  const base_date = (req.query['date'] && validator.isDate(req.query['date']))
...
const getDepartmentIdForTeamView = (req) => {
   ...
   * transition between different pages does not reset the value
   */
  let departmentId = (req.query['department'] && validator.isNumeric(req.query['department']))
