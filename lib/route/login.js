...
module.exports = function(passport) {
  ...
  const ensure_flash_on_session = (req, type, message) => {
    if (!req.session) {
      return;
    }
    req.session.flash = req.session.flash || {};
    const key = type === 'errors' ? 'errors' : 'messages';
    if (!Array.isArray(req.session.flash[key])) {
      req.session.flash[key] = [];
    }
    req.session.flash[key].push(message);
  };
  ...
  router.post('/login', function(req, res, next) {
    passport.authenticate('local', function(err, user) {
      ...
      .then(() => {
        ensure_flash_on_session(req, 'messages', 'Welcome back '+user.name+'!');
  ...
  router.post('/register', function(req, res, next){
      ...
      .then(function(){
        ensure_flash_on_session(req, 'messages', 'Registration is complete.');
        return res.redirect_with_session('/');
      })
