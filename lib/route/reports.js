...
router.get('/allowancebytime/', (req, res) => {
  ...
  let start_date = (req.query['start_date'] && validator.isDate(req.query['start_date']))
    ? moment.utc(req.query['start_date'])
    : req.user.company.get_today();

  let end_date = (req.query['end_date'] && validator.isDate(req.query['end_date']))
  ...
  var current_deparment_id  = (req.query['department'] && validator.isNumeric(req.query['department']))
...
const extractParametersForLeavesReport = ({req, actingUser}) => {
  const startDate = (req.query['start_date'] && validator.isDate(req.query['start_date']))
    ? moment.utc(req.query['start_date'])
    : actingUser.company.get_today().startOf('month');

  const endDate = (req.query['end_date'] && validator.isDate(req.query['end_date']))
    ? moment.utc(req.query['end_date'])
  ...
  const leaveTypeId = (req.query['leave_type'] && validator.isNumeric(req.query['leave_type']))
