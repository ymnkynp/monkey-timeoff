...
router.post('/company/integration-api/', (req, res) => {
  const featureIsEnabled = req.body.integration_api_enabled ? validator.toBoolean(req.body.integration_api_enabled) : false;
...
function get_and_validate_ldap_auth_configuration(args) {
  ...
  ldap_auth_enabled            = req.body['ldap_auth_enabled'] ? validator.toBoolean(req.body['ldap_auth_enabled']) : false,
  allow_unauthorized_cert      = req.body['allow_unauthorized_cert'] ? validator.toBoolean(req.body['allow_unauthorized_cert']) : false,
