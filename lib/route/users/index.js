...
router.get('/edit/:user_id/calendar/', async (req, res) => {
  ...
  const year = (req.query['year'] && validator.isNumeric(req.query['year']))
...
router.all('/search/', function(req, res){
  ...
  var email = typeof rawEmail === 'string' ? validator.trim(rawEmail).toLowerCase() : '';
...
router.get('/', function(req, res) {
    ...
    const department_id = typeof departmentParam === 'string' ? validator.trim(departmentParam) : undefined;
    var users_filter = {},
        model = req.app.get('db_model');

    if (department_id && validator.isNumeric(department_id)) {
...
function get_and_validate_user_parameters(args) {
    ...
    const safeTrim = (val) => typeof val === 'string' ? validator.trim(val) : '';

    // Get user parameters
    var name     = safeTrim(req.body['name']),
        lastname = safeTrim(req.body['lastname']),
        email    = safeTrim(req.body['email_address']),
        department_id     = safeTrim(req.body['department']),
        start_date        = safeTrim(req.body['start_date']),
        end_date          = safeTrim(req.body['end_date']),
        adjustment        = safeTrim(req.body['adjustment']),
        password          = safeTrim(req.body['password_one']),
        password_confirm  = safeTrim(req.body['password_confirm']),
        admin             = req.body['admin'] ? validator.toBoolean(req.body['admin']) : false,
        auto_approve      = req.body['auto_approve'] ? validator.toBoolean(req.body['auto_approve']) : false;
